...
import { BROWSER_TOOL_NAMES } from "@zog/contracts";
...
test("production MCP controls one persistent Electron page across visibility changes", async () => {
  ...
  const home = mkdtempSync(join(tmpdir(), "zog-visible-browser-e2e-"));
  ...
  try {
    ...
    await electronApp.evaluate(() => {
      (
        globalThis as typeof globalThis & {
          __zogVisibleBrowserE2E: { setPanelRevealEnabled(enabled: boolean): void };
        }
      ).__zogVisibleBrowserE2E.setPanelRevealEnabled(false);
    });
    ...
    await electronApp.evaluate(() => {
      ...
      ).__zogVisibleBrowserE2E.setPanelRevealEnabled(true);
    });
    ...
    const runtimeCount = await electronApp.evaluate(() => {
      const manager = (
        globalThis as typeof globalThis & {
          __zogVisibleBrowserE2E: { browserManager: { runtimes: Map<string, unknown> } };
        }
