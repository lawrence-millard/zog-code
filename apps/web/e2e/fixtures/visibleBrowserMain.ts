...
import type { BrowserAnnotationEvent, ThreadBrowserState, ThreadId } from "@zog/contracts";
...
const pipePath = process.env.ZOG_BROWSER_HOST_PIPE_PATH;
const capability = process.env.ZOG_BROWSER_HOST_CAPABILITY;
const shellPath = process.env.ZOG_E2E_SHELL_PATH;
const threadId = process.env.ZOG_E2E_THREAD_ID as ThreadId | undefined;
const zogHome = process.env.ZOG_HOME;
const annotationPreloadPath = process.env.ZOG_E2E_BROWSER_ANNOTATION_PRELOAD;

if (!pipePath || !capability || !shellPath || !threadId || !zogHome || !annotationPreloadPath) {
  throw new Error("The visible-browser Electron fixture requires its isolated E2E environment.");
}

app.setPath("userData", path.join(zogHome, "electron-userdata"));
...
function setPanelVisible(visible: boolean): void {
  ...
  pushState();
  mainWindow?.webContents.send("zog-e2e:open-panel");
}
function pushState(): void {
  if (shellReady && latestState && mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send("zog-e2e:browser-state", latestState);
  }
}
...
ipcMain.on("zog-e2e:shell-ready", () => {
  shellReady = true;
  pushState();
});

ipcMain.handle(
  "zog-e2e:attach-webview",
