...
import type { BrowserAnnotationEvent, BrowserAnnotationTheme } from "@zog/contracts";
...
test("a real Electron guest commits and reprojects a continuous annotation session", async () => {
  const mainPath = process.env.ZOG_E2E_ELECTRON_MAIN;
  ...
  const home = mkdtempSync(join(tmpdir(), "zog-browser-annotations-e2e-"));
  ...
  const electronApp = await electron.launch({
    ...
    env: {
      ...
      HOME: home,
      ZOG_HOME: home,
      ZOG_BROWSER_HOST_PIPE_PATH: pipePath,
      ZOG_BROWSER_HOST_CAPABILITY: capability,
      ZOG_E2E_SHELL_PATH: shellPath,
      ZOG_E2E_THREAD_ID: threadId,
      ZOG_E2E_BROWSER_ANNOTATION_PRELOAD: annotationPreloadPath,
    },
  });
  ...
  try {
    ...
    const sendNativeInput = (event: Record<string, unknown>) =>
      electronApp.evaluate(
        (_electron, input) => {
          ...
          ).__zogVisibleBrowserE2E.browserManager;
    ...
    const annotationEvents = async (): Promise<BrowserAnnotationEvent[]> =>
      electronApp.evaluate(() => {
        const fixture = (
          globalThis as typeof globalThis & {
            __zogVisibleBrowserE2E: { annotationEvents: BrowserAnnotationEvent[] };
          }
        ).__zogVisibleBrowserE2E;
