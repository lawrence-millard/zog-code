...
describe("store facade", () => {
  ...
  it("renames a project locally without changing its remote or folder names", () => {
    ...
    const next = renameProjectLocally(state, ProjectId.makeUnsafe("project-1"), "zog");
  ...
  it("persists project aliases immediately when the local alias changes", async () => {
    ...
    try {
      ...
      freshStore.useStore.getState().renameProjectLocally(projectId, "zog");

      expect(setItem).toHaveBeenCalled();
      expect(JSON.parse(storage.get("zog:renderer-state:v8") ?? "{}")).toMatchObject({
        projectNamesByCwd: {
          "/tmp/project": "zog",
        },
      });
