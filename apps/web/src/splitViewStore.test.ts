...
describe("splitViewStore", () => {
  ...
  it("keeps writing split views to the Zog v1 storage key so persisted state can migrate", async () => {
    vi.resetModules();
    globalThis.localStorage = createMemoryStorage();
    ...
    const persisted = globalThis.localStorage.getItem("zog:split-view-state:v1");
    expect(persisted).not.toBeNull();
    expect(globalThis.localStorage.getItem("zog:split-view-state:v2")).toBeNull();
