...
// Purpose: Routes native macOS AppSnaps into the correct Zog composer draft.
    ...
    async (capture: DesktopAppSnapCapture) => {
      ...
      if (resolvedTarget.kind === "existing") {
        ...
        if (result.threadId) {
          ...
          const focused = focusedTargetRef.current;
          if (!focused) throw new Error("Zog could not create a task for this AppSnap.");
          target = focused;
          openChatThreadPage(target.threadId);
        }
      }
      ...
      if (!image) throw new Error(error ?? "Zog could not attach the captured AppSnap.");
      ...
      toastManager.add({
        ...
        description:
          persistenceResult === "unverified"
            ? "The capture is attached, but Zog could not verify its draft metadata. If it is missing after a reload, Zog will attach it again."
