...
import { normalizeModelSlug } from "@zog/shared/model";
import { buildZogBranchName } from "@zog/shared/git";
import { isGenericChatThreadTitle } from "@zog/shared/chatThreads";
import { isGenericTerminalThreadTitle } from "@zog/shared/terminalThreads";
import {
  type ChatMessage,
  type SessionPhase,
  type Thread,
...
export const LAST_INVOKED_SCRIPT_BY_PROJECT_KEY = "zog:last-invoked-script-by-project";
export const DISMISSED_PROVIDER_HEALTH_BANNERS_KEY = "zog:dismissed-provider-health-banners";
...
export function describeVoiceRecordingStartError(error: unknown): string {
  ...
  if (errorName === "NotAllowedError" || errorName === "PermissionDeniedError") {
    return "Microphone access was denied. Enable it in macOS Privacy & Security > Microphone for Zog, then try again.";
  }
...
export function buildSuggestedWorktreeName(input: {
  ...
  return buildZogBranchName(input.associatedWorktreeBranch ?? input.title);
}
