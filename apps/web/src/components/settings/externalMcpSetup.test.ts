...
describe("external MCP guided setup", () => {
  it("builds copyable Codex and Claude Code commands without embedding a credential", () => {
    ...
    expect(codex.value).toBe(
      "codex mcp add zog --env ELECTRON_RUN_AS_NODE=1 -- /Applications/Zog.app/Contents/MacOS/Zog server.js mcp serve --integration mcp_int_example --home-dir '/tmp/Zog home'",
    );
    expect(claude.value).toBe(
      "claude mcp add --scope user zog -e ELECTRON_RUN_AS_NODE=1 -- /Applications/Zog.app/Contents/MacOS/Zog server.js mcp serve --integration mcp_int_example --home-dir '/tmp/Zog home'",
    );
  ...
  it("builds standard JSON configuration for desktop and other clients", () => {
    ...
    const parsed = JSON.parse(desktop.value) as {
      mcpServers: { zog: { command: string; args: ReadonlyArray<string> } };
    };

    expect(desktop.format).toBe("json");
    expect(parsed.mcpServers.zog).toEqual(stdio);
  });
  ...
  it("builds a project-specific prompt without exposing implementation identifiers", () => {
    const prompt = buildExternalMcpExamplePrompt("Zog app");

    expect(prompt).toContain('project named "Zog app"');
  ...
  it("builds one agent-facing setup prompt covering pairing, registration, and verification", () => {
    const prompt = buildExternalMcpSetupPrompt({
      setupCommand: "zog mcp pair --code syn_pair_v1_example --home-dir /tmp/home",
      stdio,
    });

    expect(prompt).toContain("syn_pair_v1_example");
    expect(prompt).toContain("codex mcp add zog");
    expect(prompt).toContain("claude mcp add --scope user zog");
    expect(prompt).toContain('"mcpServers"');
    expect(prompt).toContain("zog_overview");
