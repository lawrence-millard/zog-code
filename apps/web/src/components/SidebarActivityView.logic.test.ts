...
describe("project filter", () => {
  ...
  it("merges every project-less chat container into one Zog scope", () => {
...
describe("resolveActivityScope", () => {
  ...
  it("expands the Zog chats scope to its container projects", () => {
...
describe("resolveThreadProjectLabel", () => {
  it("uses the project name for real projects and Zog otherwise", () => {
    expect(
      resolveThreadProjectLabel({ kind: "project", name: "Zog App", folderName: "zog" }),
    ).toBe("Zog App");
    expect(resolveThreadProjectLabel({ kind: "chat", name: "Chats", folderName: "chats" })).toBe(
      "Zog",
    );
    expect(resolveThreadProjectLabel(undefined)).toBe("Zog");
  });
});
