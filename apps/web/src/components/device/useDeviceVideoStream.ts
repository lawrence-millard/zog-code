...
import type { DeviceUdid } from "@zog/contracts";
import type { DeviceFrame } from "@zog/shared/deviceFrame";
...
export function useDeviceVideoStream(input: {
  ...
  useEffect(() => {
    ...
    const configureDecoder = (frame: DeviceFrame) => {
      ...
      if (!codec) {
        failStream("The simulator stream sent parameters Zog could not read.");
        return;
      }
      teardownDecoder();
      ...
      setStatus({
        ...
        message:
          reason === "decode-failed"
            ? "The simulator stream sent a frame Zog could not read."
