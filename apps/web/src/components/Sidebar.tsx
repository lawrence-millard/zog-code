...
import { isGenericChatThreadTitle } from "@zog/shared/chatThreads";
import { getDefaultModel } from "@zog/shared/model";
...
type DebugFeatureFlagsWindow = Window & {
  zogShowFeatureFlags?: () => void;
  zogHideFeatureFlags?: () => void;
};
...
const ZOG_DOCS_URL = "https://code.zog.watch/docs";
      ...
      <Menu>
        ...
        <ComposerPickerMenuPopup align="end" side="top" className="w-64 min-w-64">
          ...
          <MenuGroup>
            ...
            <MenuItem
              className={SIDEBAR_CONTEXT_MENU_ITEM_CLASS_NAME}
              onClick={() => openExternalLink(ZOG_DOCS_URL)}
            >
 ...
 */
const ACTIVITY_ONBOARDING_STORAGE_KEY = "zog:activity-onboarding:v1";
...
const SIDEBAR_SURFACE_PICKER_COPY: Record<SidebarView, { title: string; description: string }> = {
  threads: { title: "Zog", description: "Build, debug, and ship" },
...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    debugWindow.zogShowFeatureFlags = showFeatureFlags;
    debugWindow.zogHideFeatureFlags = hideFeatureFlags;
    window.addEventListener("storage", updateVisibility);
    updateVisibility();

    return () => {
      window.removeEventListener("storage", updateVisibility);
      if (debugWindow.zogShowFeatureFlags === showFeatureFlags) {
        delete debugWindow.zogShowFeatureFlags;
      }
      if (debugWindow.zogHideFeatureFlags === hideFeatureFlags) {
        delete debugWindow.zogHideFeatureFlags;
      }
    };
  }, []);
  ...
  // "Zog" for project-less chats) with the hover cards and Activity rows.
  ...
  function renderProjectItem(
    ...
    // A project reads as "running" when Zog tracks a run for it or when a
    // local server (possibly started outside Zog) is attributed by cwd.
  ...
  const searchPaletteActions = useMemo<SidebarSearchAction[]>(
    () => [
      ...
      },
      {
        id: "feedback",
        label: "Feedback Zog",
        description: "Send feedback or report an issue to the Zog team.",
        keywords: ["feedback", "bug", "issue", "problem", "report", "support", "zog"],
      },
      {
        id: "settings",
        label: "Settings",
        ...
        .then((nextState) => {
          ...
          if (nextState.status === "available") {
            toastManager.add({
              ...
              description: `Zog is preparing version ${nextState.availableVersion ?? "available"} in the background.`,
            });
            return;
          }
          ...
          if (nextState.status === "downloading") {
            toastManager.add({
              type: "info",
              title: "Preparing update",
              description: "Zog is downloading the update in the background.",
            });
            return;
          }
          ...
          if (nextState.status === "up-to-date") {
            toastManager.add({
              ...
              description: `Zog ${nextState.currentVersion} is already the newest version.`,
            });
            return;
          }
