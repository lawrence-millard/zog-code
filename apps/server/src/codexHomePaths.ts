...
export const ZOG_CODEX_HOME_OVERLAY_DIR = "codex-home-overlay";
...
export function resolveZogCodexHomeOverlayPath(
  env: NodeJS.ProcessEnv,
  sourceHomePath: string,
): string {
  const runtimeHome = env.ZOG_HOME?.trim();
  const overlayRoot = runtimeHome || path.join(path.dirname(sourceHomePath), ".zog", "runtime");
  return path.join(overlayRoot, ZOG_CODEX_HOME_OVERLAY_DIR);
}

/**
 * Returns the home directory that the codex app-server child process actually
 * writes under. Zog keeps its generated config isolated from the user's
...
export function resolveActiveCodexHomeWritePath(input: CodexHomePathsInput = {}): string {
  ...
  const overlay = resolveZogCodexHomeOverlayPath(env, source);
