...
import { isTemporaryWorktreeBranch } from "@zog/shared/git";
  ...
  const gitLayer = Layer.succeed(GitCore, {
    ...
    createDetachedWorktree: (input: { ref: string; path?: string; newBranch?: string }) =>
      Effect.gen(function* () {
        ...
        return {
          worktree: {
            path: input.path ?? "/tmp/worktrees/generated/zog",
            ref: input.ref,
...
describe("AgentGateway", () => {
    ...
    return Effect.gen(function* () {
      ...
      const response = yield* harness.postRaw({
        ...
        body: {
          ...
          id: true,
          method: "tools/call",
          params: { name: "zog_set_thread_title", arguments: { title: "Must not run" } },
        },
      });
  ...
  it.effect("answers initialize with instructions and lists tools", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.notInclude(String(initResult.instructions), "[Zog harness policy");
      ...
      const createThreadProperties = tools.find((tool) => tool.name === "zog_create_thread")
      ...
      const createThreadsTool = tools.find((tool) => tool.name === "zog_create_threads");
      ...
      const createAutomation = tools.find((tool) => tool.name === "zog_create_automation");
      ...
      const updateAutomationMemory = tools.find(
        (tool) => tool.name === "zog_update_automation_memory",
      );
      const reportAutomationResult = tools.find(
        (tool) => tool.name === "zog_report_automation_result",
      );
      assert.include(
      ...
      const updateAutomationProperties = tools.find(
        (tool) => tool.name === "zog_update_automation",
      ...
      extraProjects: [
        {
          ...
          kind: "chat",
          title: "che progetti ci sono in zog",
          workspaceRoot: `${homeDir}/Documents/Zog/2026-03-01/chat`,
        },
        {
          ...makeProjectShell(),
          ...
          kind: "studio",
          title: "Studio",
          workspaceRoot: `${homeDir}/Documents/Zog/Studio`,
        },
        {
          ...makeProjectShell(),
  ...
  it.effect("fetches a GitHub pull-request head without creating a local branch", () => {
    ...
    return Effect.gen(function* () {
      ...
      // The worktree is born on a temporary zog/* branch, but no branch is
  ...
  it.effect("rejects every destructive tool after the caller turn completes", () => {
    ...
    return Effect.gen(function* () {
      ...
      const attempts = [
        ...
        { name: "zog_interrupt_thread", args: { threadId: "thread-child" } },
        {
    ...
    return Effect.gen(function* () {
      ...
      const creationRecaps = harness.dispatched.filter(
        (command) =>
          ...
          command.activity.kind === "zog.threads.created",
      );
      ...
      assert.deepEqual(
        ...
        [0, 1].map((index) => ({
          creationSource: "zog_mcp" as const,
          sourceThreadId: ThreadId.makeUnsafe("thread-parent"),
          sourceTurnId: TurnId.makeUnsafe("turn-parent-active"),
