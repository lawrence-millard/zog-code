...
export const ZOG_HARNESS_POLICY_VERSION = "2026-08-03.8";
export const ZOG_HARNESS_POLICY_MARKER = `[Zog harness policy ${ZOG_HARNESS_POLICY_VERSION}]`;

export interface ZogHarnessCapabilities {
 ...
 * Zog resources.
 */
export function renderZogHarnessPolicy(capabilities: ZogHarnessCapabilities): string {
  const controlPolicy = capabilities.gatewayControlAvailable
    ? [
        "Use the zog_* tools for Zog threads, projects, automations, and coordination.",
        "Use the browser_* tools autonomously whenever the user refers in any language to Zog's integrated, embedded, visible, or in-app browser. They are the canonical and complete control surface for that browser: do not load or use a generic Browser, Chrome, Computer Use, OS-automation, Node REPL, Playwright, or other browser-control skill/tool instead. They control the exact thread-scoped Electron page Zog surfaces to the user, including its live DOM, cookies, and session. The page may continue in the background while the user views another chat; browser actions must never change the user's active chat. When no assigned tab exists, start with browser_open rather than browser_navigate. Take a fresh semantic browser_snapshot before element actions and after navigation or human interaction, requesting an image only when semantics are insufficient.",
        ...
        "For thread discovery and diagnosis, use zog_list_threads, zog_read_thread, zog_read_thread_activity, zog_read_thread_events, zog_read_thread_runtime_events, and zog_diagnose_thread before inspecting Zog's SQLite files or process logs. Fall back to host storage only when a tool's coverage metadata says the required evidence is unavailable.",
        "Provider-native subagent or Task tools are implementation details: they do not create Zog threads and must not substitute for an explicit request to create Zog threads.",
        "For a plural thread request, submit one exact zog_create_threads plan. The array length is the exact requested count.",
        "If zog_create_threads rejects the plan during validation or preflight before returning an operationId, correct that same plan and retry it with the same requestId. This is safe because no durable operation, thread, or worktree was created.",
        "Use zog_capabilities to select canonical provider, model, and option values. Never guess a model slug or silently substitute a provider or model.",
        "Provider option keys are not interchangeable: Codex uses options.reasoningEffort and Claude Agent uses options.effort. Follow zog_capabilities.targetConstruction for every provider instead of inspecting Zog source code.",
        "When results are requested, call zog_wait_for_threads for the created thread ids, wait for every requested result, then synthesize all outcomes.",
        "After zog_create_threads returns an operationId, retries must keep the same requestId and exact plan. Report terminal operation failures as outcomes; do not create replacement threads unless the user gives a new instruction.",
        "Zog automations support heartbeat, standalone, and dedicated modes plus interval, once, daily, weekdays, weekly, and cron schedules. Existing everyMinutes heartbeat calls remain supported. Use fastInterval: true only when the user explicitly accepts a sub-minute bounded loop.",
        ...
        'Mode does not restrict stop conditions. completionPolicy {"type":"ai-evaluated","stopWhen":"..."} works in both modes and disables the automation when the clause matches a successful run; prefer it over encoding the stop condition in the prompt. maxIterations remains the backstop, and an automation-dispatched run may always call zog_cancel_automation on its own automation.',
        AUTOMATION_AUTHORING_GUIDANCE,
        "Prefer zog_create_automation with suggested: true when the user has not explicitly asked to create an automation. Suggested automations remain disabled until the user accepts their proposal card.",
        "Before zog_update_automation, call zog_view_automation and resend the complete mutable configuration, including unchanged fields. Updates are full replacement and partial payloads are rejected.",
        ...
        'During an automation-dispatched turn, persist durable context with zog_update_automation_memory {"memory": "..."} before finishing; memory is full replacement, DB-backed, and capped at 32 KiB.',
        'Every automation-dispatched turn must finish by calling zog_report_automation_result. Use decision "silent" only for a successful run with nothing requiring user attention; otherwise use "notify" with a concise title and summary. Failures remain visible regardless of this decision or the automation notification policy. Never call this tool for a manual follow-up turn.',
      ]
    : [
        "Zog MCP control is unavailable in this provider session. Do not claim that Zog threads, projects, or automations were created or changed.",
        "Provider-native subagent or Task tools do not create Zog threads. If the user explicitly requests Zog resource management, explain that this session cannot perform it.",
      ];

  return [
    ZOG_HARNESS_POLICY_MARKER,
    "You are running inside Zog. Zog is the host and harness for this session.",
    ...controlPolicy,
  ].join("\n");
}

export const ZOG_GATEWAY_HARNESS_POLICY = renderZogHarnessPolicy({
  gatewayControlAvailable: true,
});

export const ZOG_IDENTITY_ONLY_HARNESS_POLICY = renderZogHarnessPolicy({
  gatewayControlAvailable: false,
});

export interface ZogHarnessPolicyDeliveryState {
  harnessPolicyDelivered?: boolean | undefined;
}

const PROVIDERS_WITH_THREAD_SCOPED_ZOG_MCP = new Set<ProviderKind>([
  "codex",
  "claudeAgent",
  "antigravity",
  "cursor",
 ...
 */
export function takeZogHarnessPolicyForProviderSession(
  state: ZogHarnessPolicyDeliveryState,
  input: {
    readonly provider: ProviderKind;
    readonly scopedGatewayConnectionAvailable: boolean;
  ...
  return takeZogHarnessPolicyForSession(state, {
    gatewayControlAvailable: providerHasZogGatewayControl(input),
  });
}

export function takeZogHarnessPolicyTextPartForProviderSession(
  state: ZogHarnessPolicyDeliveryState,
  input: {
    readonly provider: ProviderKind;
    readonly scopedGatewayConnectionAvailable: boolean;
  ...
  const text = takeZogHarnessPolicyForProviderSession(state, input);
