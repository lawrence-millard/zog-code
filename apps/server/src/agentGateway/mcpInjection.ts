/**
 * Provider-facing config builders for the Zog agent gateway.
 *
 ...
 *
 * - Codex: `[mcp_servers.zog]` TOML block (streamable HTTP +
...
export const ZOG_MCP_SERVER_NAME = "zog";
export const ZOG_AGENT_GATEWAY_TOKEN_ENV = "ZOG_AGENT_GATEWAY_TOKEN";
export const ZOG_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV = "ZOG_AGENT_GATEWAY_BOOTSTRAP_TOKEN";
export const ZOG_AGENT_GATEWAY_URL_ENV = "ZOG_AGENT_GATEWAY_URL";
 ...
 * the block references an env var that Zog sets per app-server process.
 *
...
export function buildCodexMcpConfigToml(endpointUrl: string): string {
  return [
    `[mcp_servers.${ZOG_MCP_SERVER_NAME}]`,
    `url = ${JSON.stringify(endpointUrl)}`,
    `bearer_token_env_var = ${JSON.stringify(ZOG_AGENT_GATEWAY_TOKEN_ENV)}`,
    "",
    "[shell_environment_policy]",
    `exclude = [${JSON.stringify(ZOG_AGENT_GATEWAY_TOKEN_ENV)}]`,
  ].join("\n");
}
 ...
 * provider process dedicated to the owning Zog thread or an exclusive
...
async function postAgentGatewayJsonRpc(input: {
  ...
  if (!response.ok) {
    throw new Error(`Zog MCP request failed with HTTP ${String(response.status)}.`);
  }
  const payload: unknown = await response.json();
  if (!isRecord(payload) || payload.jsonrpc !== "2.0") {
    throw new Error("Zog MCP returned an invalid JSON-RPC response.");
  }
  if ("error" in payload) {
    const failure = isRecord(payload.error) ? payload.error : null;
    throw new Error(failure?.message ? String(failure.message) : "Zog MCP request failed.");
  }
  if (payload.id !== id || !("result" in payload)) {
    throw new Error("Zog MCP returned a mismatched JSON-RPC response.");
  }
...
export async function listAgentGatewayMcpTools(input: {
  ...
  if (!isRecord(result) || !Array.isArray(result.tools)) {
    throw new Error("Zog MCP tools/list returned an invalid tool catalog.");
  }
  return result.tools.map((value) => {
    if (
      ...
      throw new Error("Zog MCP tools/list returned an invalid tool descriptor.");
    }
    return {
      name: value.name,
...
/**
 * Build the secret-free MCP fragment installed with Zog's Antigravity
  ...
  return {
    mcpServers: {
      [ZOG_MCP_SERVER_NAME]: {
        ...
        env: {
          [ZOG_AGENT_GATEWAY_URL_ENV]: `$${ZOG_AGENT_GATEWAY_URL_ENV}`,
          [ZOG_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]: `$${ZOG_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV}`,
 ...
 */
export function buildAcpZogMcpServers(input: {
  readonly connection: AgentGatewayMcpConnection;
  ...
  return [
    {
      ...
      env: [
        { name: ZOG_AGENT_GATEWAY_URL_ENV, value: input.connection.url },
        { name: ZOG_AGENT_GATEWAY_TOKEN_ENV, value: input.connection.bearerToken },
      ],
    },
  ];
}
