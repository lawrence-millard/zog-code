...
import type { ProviderRuntimeEvent } from "@zog/contracts";
...
describe("resolveCodexGeneratedImagesRoot(s)", () => {
  ...
  afterEach(() => {
    if (previousZogHome === undefined) delete process.env.ZOG_HOME;
    else process.env.ZOG_HOME = previousZogHome;
  });

  it("returns the overlay generated_images directory as the active write root by default", () => {
    process.env.ZOG_HOME = "/zog-test/runtime";
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
      path.join("/zog-test/runtime", "codex-home-overlay", "generated_images"),
    );
  });
  ...
  it("collapses to a single root when overlay equals source", () => {
    ...
    // The overlay falls under `<dirname(source)>/.zog/runtime/codex-home-overlay`,
    ...
    const homePath = "/runtime/.zog/runtime/codex-home-overlay";
