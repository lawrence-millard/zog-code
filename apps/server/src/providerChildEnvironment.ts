...
// Purpose: Builds provider child environments without Zog control-plane authority.
...
const isTestHarnessKey = (key: string, env: NodeJS.ProcessEnv): boolean =>
  Boolean(env.VITEST) && (key.startsWith("ZOG_FAKE_") || key.startsWith("ZOG_ACP_"));

export function buildProviderChildEnvironment(input: {
  readonly provider: ProviderChildKind;
  readonly baseEnv?: NodeJS.ProcessEnv;
  readonly inheritedZogKeys?: ReadonlyArray<string>;
  ...
  const allowedZogKeys = new Set(input.inheritedZogKeys ?? []);
