...
// Purpose: Compute Profile-page stats from Zog's local projection DB only.
...
import { isBuiltInComposerSlashCommandName } from "@zog/shared/composerSlashCommands";
...
// Builds profile skill rows from every stored Zog user message, plus the
...
>()("zog/profileStats/ProfileStatsQuery") {}
...
const makeProfileStatsQuery = Effect.gen(function* () {
  ...
  // Activity = days/hours the user actually sent a Zog prompt. One day-hour
  ...
  // Token usage for EVERY provider, straight from Zog's own DB (no external
  ...
  const getProfileStats = (
    ...
    Effect.gen(function* () {
      ...
      // ── Identity ──
      const homeDirBasename = nodePath.basename(config.homeDir) || "zog";
