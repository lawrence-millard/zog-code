...
describe("managed checkpoint refs", () => {
  ...
  it("creates canonical Zog refs", () => {
    expect(checkpointRefForThreadTurn(threadId, 4)).toMatch(/^refs\/zog\/checkpoints\//);
  });
  ...
  it("recognizes a structurally valid persisted ref for the same thread", () => {
    ...
    const historical = canonical.replace("refs/zog/", "refs/historical/");
