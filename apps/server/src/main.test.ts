...
import { CliConfig, makeServerStartupLogData, zogCli, type CliConfigShape } from "./main";
...
function makeTempHome(prefix = "zog-main-test-"): string {
...
const runCli = (args: ReadonlyArray<string>, env: Record<string, string> = {}) => {
  const program = Command.runWith(zogCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            ZOG_HOME: defaultZogHome,
            ZOG_NO_BROWSER: "true",
            ...env,
          },
        }),
...
it.layer(testLayer)("server CLI command", (it) => {
  it.effect("parses all CLI flags and wires scoped start/stop", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("zog-main-flag-");
  ...
  it.effect("passes the root --home-dir flag to MCP subcommands", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("zog-main-mcp-flag-");
  ...
  it.effect("creates fresh local state directories with private permissions", () =>
    Effect.gen(function* () {
      ...
      const homeDir = makeTempHome("zog-main-private-fresh-");
  ...
  it.effect("repairs permissions for an upgraded local state directory", () =>
    Effect.gen(function* () {
      if (process.platform === "win32") return;
      const homeDir = makeTempHome("zog-main-private-upgrade-");
      const stateDir = path.join(homeDir, "userdata");
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      const envHome = makeTempHome("zog-main-env-");
      ...
      yield* runCli([], {
        ...
        ZOG_NO_BROWSER: "true",
        ZOG_AUTH_TOKEN: "env-token",
        ZOG_DESKTOP_SHUTDOWN_TOKEN: "shutdown-token",
      });
  ...
  it.effect("consumes desktop shutdown authority before generic child launches", () =>
    Effect.gen(function* () {
      const canonicalKey = "ZOG_DESKTOP_SHUTDOWN_TOKEN";
      ...
      try {
        ...
        resolvedConfig = null;
        yield* runCli([], { ZOG_DESKTOP_SHUTDOWN_TOKEN: injectedToken });
  ...
  it.effect("omits both server authority secrets from startup log data", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        ZOG_AUTH_TOKEN: "browser-secret",
        ZOG_DESKTOP_SHUTDOWN_TOKEN: "shutdown-secret",
      });
  ...
  it.effect("prefers --mode over ZOG_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over ZOG_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        ZOG_NO_BROWSER: "false",
      });
    ...
    Effect.gen(function* () {
      yield* runCli(
        ...
        ],
        {
          ZOG_MODE: "desktop",
          ZOG_NO_BROWSER: "true",
          ZOG_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
          ZOG_LOG_PROVIDER_EVENTS: "true",
          ZOG_LOG_WS_EVENTS: "true",
        },
      );
  ...
  it.effect("honors insecure remote opt-in from the environment when the CLI flag is absent", () =>
    Effect.gen(function* () {
      yield* runCli(["--host", "0.0.0.0", "--auth-token", "remote-secret"], {
        ZOG_ALLOW_INSECURE_REMOTE: "true",
        ZOG_NO_BROWSER: "true",
      });
    ...
    Effect.gen(function* () {
      yield* runCli(
        [
          ...
          "remote-secret",
          "--public-url",
          "https://zog.example.test",
        ],
        { ZOG_NO_BROWSER: "false" },
      );

      assert.equal(resolvedConfig?.publicUrl?.origin, "https://zog.example.test");
      assert.equal(openBrowser.mock.calls.length, 1);
      assert.match(
        openBrowser.mock.calls[0]?.[0] ?? "",
        /^https:\/\/zog\.example\.test\/pair#token=/,
      );
    }),
  );
  ...
  it.effect("supports the HTTPS public origin through environment configuration", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        ZOG_HOST: "192.168.1.50",
        ZOG_AUTH_TOKEN: "remote-secret",
        ZOG_PUBLIC_URL: "https://zog.example.test",
      });
  ...
  it.effect("rejects non-root or non-HTTPS public URLs", () =>
    Effect.gen(function* () {
      for (const publicUrl of ["http://zog.example.test", "https://zog.example.test/app"]) {
        const error = yield* Effect.flip(
  ...
  it.effect("refuses non-loopback exposure without authentication", () =>
    Effect.gen(function* () {
      ...
      assert.match(String(error), /Refusing to bind Zog to non-loopback host 0\.0\.0\.0/);
    }),
  );
  ...
  it.effect("supports CLI and env for bootstrap/provider-log/websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        ZOG_MODE: "desktop",
        ZOG_LOG_PROVIDER_EVENTS: "true",
        ZOG_LOG_WS_EVENTS: "false",
        ZOG_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        ZOG_NO_BROWSER: "true",
      });
  ...
  it.effect("rejects invalid boolean environment values instead of treating them as absent", () =>
    Effect.gen(function* () {
      const error = yield* Effect.flip(
        runCli([], {
          ZOG_LOG_PROVIDER_EVENTS: "sometimes",
        }),
      );
