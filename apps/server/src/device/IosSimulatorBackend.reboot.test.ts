...
function makeBackend() {
  ...
  const backend = new IosSimulatorBackend({
    platform: "darwin",
    helperCacheRoot: "/tmp/zog-device-test-cache",
  ...
  Object.defineProperty(backend, "compileHelperIfNeeded", {
    value: async () => "/tmp/zog-device-test-cache/zog-device-helper",
  });
...
describe("simulator reboot", () => {
  it("re-attaches after a Zog-driven shutdown and reboot", async () => {
  ...
  it("recovers when the device was rebooted outside Zog", async () => {
    const { backend, helper } = makeBackend();
    await backend.tap(DEVICE, 10, 10);
