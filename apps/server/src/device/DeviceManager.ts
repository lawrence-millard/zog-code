 ...
 * - The Zog boot cap (`DEVICE_ZOG_BOOT_LIMIT`). Boot past the cap is
...
/** How long a Zog-booted device stays up with no thread attached. */
...
export interface DeviceManagerOptions {
  ...
  /** Remembers Zog's boots across a crash; defaults to remembering nothing. */
...
export class DeviceManager {
  ...
  private readonly zogBooted = new Set<string>();
  ...
  constructor(options: DeviceManagerOptions) {
    ...
    this.bootLimit = options.bootLimit ?? DEVICE_ZOG_BOOT_LIMIT;
  ...
  private async recordBootOwnership(): Promise<void> {
    await this.bootOwnership.write([...this.zogBooted]).catch(() => undefined);
  }
  ...
  async zogBootedDevices(): Promise<readonly DeviceDescriptor[]> {
    const devices = await this.backend.listDevices({ includeShutdown: true }).catch(() => []);
    this.reconcileZogBooted(devices);
    return devices
      .filter((device) => this.zogBooted.has(device.udid))
   ...
   *
   * The set is Zog's own bookkeeping, but the simulators are not Zog's to
   ...
   */
  private reconcileZogBooted(devices: readonly DeviceDescriptor[]): void {
    const running = new Set(
      devices
        .filter((device) => device.state === "booted" || device.state === "booting")
        .map((device) => device.udid),
    );
    for (const udid of this.zogBooted) {
      if (running.has(udid)) continue;
      this.zogBooted.delete(udid);
  ...
  async boot(udid: string): Promise<DeviceBootResult> {
    const devices = await this.backend.listDevices({ includeShutdown: true }).catch(() => []);
    // Devices that stopped without Zog doing it still held their slots, so
    ...
    // Zog from accumulating simulators, not to limit what the user watches.
    if (known?.state === "booted") {
      return { kind: "booted", device: this.describe(known) };
    }
    if (this.zogBooted.size >= this.bootLimit) {
      return {
        kind: "boot-limit-reached",
        limit: this.bootLimit,
        zogBooted: await this.zogBootedDevices(),
      };
    }
    ...
    // exists to stop Zog accumulating multi-gigabyte simulators would be
    // exceeded by however many requests arrived inside that window.
    this.zogBooted.add(udid);
    ...
    return { kind: "booted", device: { ...device, bootSource: "zog" } };
  }
  ...
  /**
   * App quit: shut down everything Zog booted, leave the user's devices
  ...
  async dispose(): Promise<void> {
    ...
    const booted = Array.from(this.zogBooted);
  ...
  private describe(device: DeviceDescriptor): DeviceDescriptor {
    const bootSource = this.zogBooted.has(device.udid) ? "zog" : device.bootSource;
   ...
   * the device down or start the idle countdown if Zog booted it.
   *
   * A switch shuts down immediately rather than waiting out the idle window.
   * The cap is three Zog-booted devices, and each one costs a couple of GB
  ...
  private async releaseDevice(
    ...
    if (!this.zogBooted.has(udid)) return;
    if (reason === "switched") {
      this.clearIdleTimer(udid);
  ...
  private async shutdownIfStillIdle(udid: string): Promise<void> {
    ...
    if (this.isAttachedAnywhere(udid) || !this.zogBooted.has(udid)) return;
    await this.shutdown(udid).catch(() => undefined);
  }
