 ...
 * directory so both land in the same place under a custom ZOG_HOME.
 *
...
function defaultBootOwnershipPath(): string {
  const baseDir = process.env.ZOG_HOME?.trim() || path.join(homedir(), ".zog");
          ...
          if (reclaimed.length > 0) {
            console.info(
              `[device] shut down ${reclaimed.length} simulator(s) left booted by a previous ` +
                `Zog run: ${reclaimed.join(", ")}`,
            );
          }
        });
      }

      // App quit shuts down every simulator Zog booted and leaves the
