...
import { parseGitHubRepositoryNameWithOwnerFromRemoteUrl } from "@zog/shared/githubRepository";
...
const AUTO_DETACHED_WORKTREE_DIRNAME = "zog";
const WORKTREE_OWNERSHIP_MARKER = "zog-agent-gateway-owner.json";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `zog-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
  ...
  Effect.gen(function* () {
      ...
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const tempIndexDir = yield* fileSystem.makeTempDirectoryScoped({
            prefix: `zog-git-status-index-${process.pid}-`,
          });
      ...
      Effect.gen(function* () {
        ...
        if (patch.length > 0) {
          yield* Effect.acquireUseRelease(
            Effect.tryPromise({
              try: () => nodeFs.mkdtemp(nodePath.join(tmpdir(), "zog-worktree-patch-")),
              catch: (cause) =>
                createGitCommandError(
    ...
    const removeWorktree: GitCoreShape["removeWorktree"] = (input) =>
      Effect.gen(function* () {
        ...
        // zog/* branches qualify for reclamation; detached HEADs and
    ...
    const stashAndCheckout: GitCoreShape["stashAndCheckout"] = (input) =>
      Effect.gen(function* () {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["stash", "push", "-u", "-m", `zog: stash before switching to ${input.branch}`],
          {
            timeoutMs: 30_000,
