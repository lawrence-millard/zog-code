...
import { workspaceRootsEqual } from "@zog/shared/threadWorkspace";
import {
  isThreadDetailEventFor,
  THREAD_DETAIL_EVENT_TYPES,
} from "@zog/shared/threadDetailEvents";
...
import { discoverSkillsCatalog, zogSkillsDir } from "./provider/skillsCatalog";
...
class WsRequestAdmissionMiddleware extends RpcMiddleware.Service<WsRequestAdmissionMiddleware>()(
  "zog/WsRequestAdmissionMiddleware",
          ...
          rpcEffect(
            Effect.tryPromise(() =>
              discoverSkillsCatalog({
                cwd: input.cwd ?? null,
                homeDir: config.homeDir,
                zogBaseDir: config.baseDir,
                includeDuplicateOrigins: true,
              }),
            ).pipe(
              Effect.map((skills) => ({
                skills,
                zogSkillsDir: zogSkillsDir(config.baseDir),
              })),
            ),
    ...
    Effect.gen(function* () {
      ...
      yield* router.add(
        ...
        Effect.gen(function* () {
          ...
          // The desktop app fetches cross-origin (zog://app); reflect only
