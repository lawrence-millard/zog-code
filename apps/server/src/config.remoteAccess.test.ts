...
describe("remote access policy", () => {
  ...
  it("requires authentication when an HTTPS proxy publishes a loopback bind", () => {
    ...
    ).toContain("without ZOG_AUTH_TOKEN");
  });

  it("rejects invalid public URLs in the shared embedded-server policy", () => {
    for (const publicUrl of [
      new URL("http://zog.example.test/"),
      new URL("https://zog.example.test/app"),
    ]) {
      expect(
        remoteAccessPolicyError({
          ...remoteBase,
  ...
  it("accepts only credential-free HTTPS root origins", () => {
    expect(normalizeHttpsPublicOrigin(new URL("https://zog.example.test/"))?.origin).toBe(
      "https://zog.example.test",
    );
    for (const value of [
      "http://zog.example.test/",
      "https://user:pass@zog.example.test/",
      "https://zog.example.test/app",
      "https://zog.example.test/?query=1",
      "https://zog.example.test/#fragment",
    ]) {
