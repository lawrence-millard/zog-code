...
describe("buildProviderChildEnvironment", () => {
  it("strips Zog control-plane and inherited native capabilities", () => {
    const env = buildProviderChildEnvironment({
      ...
      baseEnv: {
        ...
        ZOG_AUTH_TOKEN: "control-plane-secret",
        ZOG_BROWSER_USE_PIPE_PATH: "/tmp/browser.sock",
        ZOG_BROWSER_HOST_CAPABILITY: "private-desktop-capability",
  ...
  it("admits only explicitly granted capability keys", () => {
    const env = buildProviderChildEnvironment({
      ...
      inheritedZogKeys: ["ZOG_BROWSER_USE_PIPE_PATH"],
  ...
  it("does not let overlays bypass the capability policy", () => {
    const env = buildProviderChildEnvironment({
      ...
      overrides: {
        ...
        ZOG_AUTH_TOKEN: "overlaid-control-plane-secret",
        NODE_OPTIONS: "--require=/tmp/inject.js",
      },
    });
  ...
  it("keeps stripped authority absent in descendants", () => {
    ...
    const descendantScript =
      "process.stdout.write(JSON.stringify({ xai: process.env.XAI_API_KEY, anthropic: process.env.ANTHROPIC_API_KEY, zog: process.env.ZOG_AUTH_TOKEN }))";
