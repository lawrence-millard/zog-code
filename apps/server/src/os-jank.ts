...
import { createCachedLoginShellPathReader } from "@zog/shared/loginShellEnvironment";
import {
  expandHomePath as expandHomePathSync,
  resolveZogHomeDirectory,
} from "@zog/shared/zogHome";
...
export function fixPath(
  ...
  try {
    ...
    // Cached by default: the probe result is persisted under the Zog home and reused
...
export const resolveBaseDir = (raw: string | undefined): Effect.Effect<string> =>
  Effect.succeed(resolveZogHomeDirectory({ configuredHome: raw }));
