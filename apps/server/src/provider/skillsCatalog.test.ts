...
import type { ProviderSkillDescriptor } from "@zog/contracts";
...
beforeEach(() => {
  clearSkillsCatalogCacheForTests();
  root = mkdtempSync(path.join(os.tmpdir(), "zog-skills-catalog-"));
  homeDir = path.join(root, "home");
  zogBaseDir = path.join(homeDir, ".zog");
});
...
describe("discoverSkillsCatalog", () => {
  it("creates the Zog skills folder on first discovery", async () => {
    await discoverSkillsCatalog({ homeDir, zogBaseDir });
    await expect(access(path.join(zogBaseDir, "skills"))).resolves.toBeUndefined();
  });

  it("aggregates skills from zog and provider home folders with origin scopes", async () => {
    await writeSkill(path.join(zogBaseDir, "skills", "portable"), "portable", "Zog skill");
    ...
    const skills = await discoverSkillsCatalog({ homeDir, zogBaseDir });
    const byName = new Map(skills.map((skill) => [skill.name, skill]));

    expect(byName.get("portable")?.scope).toBe("zog");
  ...
  it("includes user and matching project Claude plugins but excludes other projects", async () => {
    ...
    const skills = await discoverSkillsCatalog({ cwd, homeDir, zogBaseDir });
  ...
  it("can include duplicate skill names from different origins for settings", async () => {
    ...
    const defaultCatalog = await discoverSkillsCatalog({ homeDir, zogBaseDir });
  ...
  it("prefers the provider-native copy and falls back to Zog for that provider", async () => {
    await writeSkill(path.join(zogBaseDir, "skills", "shared"), "shared", "Zog copy");
    await writeSkill(path.join(homeDir, ".codex", "skills", "shared"), "shared", "Codex copy");
    await writeSkill(path.join(zogBaseDir, "skills", "only-zog"), "only-zog", "Fallback");

    const codexView = await discoverSkillsCatalog({ homeDir, zogBaseDir, provider: "codex" });
    ...
    expect(codexView.some((skill) => skill.name === "only-zog")).toBe(true);

    // A provider without its own copy resolves the Zog fallback.
    ...
    expect(claudeShared?.scope).toBe("zog");
  });

  it("uses documented provider alias roots before Zog fallbacks", async () => {
    await writeSkill(path.join(zogBaseDir, "skills", "shared"), "shared", "Zog copy");
...
`,
  ...
  it("serves cached results within the TTL and rescans on forceReload", async () => {
    await writeSkill(path.join(zogBaseDir, "skills", "first"), "first", "First skill");

    const initial = await discoverSkillsCatalog({ homeDir, zogBaseDir });
    expect(initial.map((skill) => skill.name)).toEqual(["first"]);

    // A skill added after the first scan is invisible to the cached entry...
    await writeSkill(path.join(zogBaseDir, "skills", "second"), "second", "Second skill");
    const cached = await discoverSkillsCatalog({ homeDir, zogBaseDir });
    expect(cached.map((skill) => skill.name)).toEqual(["first"]);

    // ...but forceReload bypasses the cache and refreshes it.
    const reloaded = await discoverSkillsCatalog({ homeDir, zogBaseDir, forceReload: true });
    expect(reloaded.map((skill) => skill.name).sort()).toEqual(["first", "second"]);
  });

  it("includes project-level .zog skills when a cwd is provided", async () => {
    const cwd = path.join(root, "repo", "packages", "web");
    await mkdir(cwd, { recursive: true });
    await writeSkill(
      path.join(root, "repo", ".zog", "skills", "repo-skill"),
      "repo-skill",
      "Project skill",
    );
  ...
  it("keeps home origins when the cwd lives under the home dir", async () => {
    ...
    await writeSkill(path.join(zogBaseDir, "skills", "portable"), "portable", "Zog");
    ...
    expect(skills.find((skill) => skill.name === "portable")?.scope).toBe("zog");
  });

  it("dedupes same-named skills within a root deterministically", async () => {
    await writeSkill(path.join(zogBaseDir, "skills", "zeta"), "twin", "Copy in zeta");
    await writeSkill(path.join(zogBaseDir, "skills", "alpha"), "twin", "Copy in alpha");
...
describe("mergeSkillsIntoCatalog", () => {
  ...
  it("keeps provider-native entries and appends catalog-only entries", () => {
    const merged = mergeSkillsIntoCatalog({
      ...
      catalog: [descriptor("Shared", "zog"), descriptor("extra", "zog")],
    });
