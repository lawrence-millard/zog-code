...
//          walking) plus the unified cross-provider skills catalog backing Zog
//          portable skills. Aggregates `~/.zog/skills` with every provider-native
...
//          mergeSkillsIntoCatalog, filterDisabledSkills, ensureZogSkillsDir

import * as fs from "node:fs/promises";
import * as nodePath from "node:path";

import type { ProviderKind, ProviderSkillDescriptor } from "@zog/contracts";
...
export interface SkillsCatalogDiscoveryInput {
  ...
  /** Zog base dir (usually `~/.zog`); skills live in `{base}/skills`. */
  readonly zogBaseDir: string;
...
export interface SkillsCatalogRootInput extends SkillsCatalogDiscoveryInput {
  /** Native provider scans can opt out; the catalog itself always includes Zog. */
  readonly includeZogRoot?: boolean;
}
...
const ensuredZogSkillsDirs = new Set<string>();

export function clearSkillsCatalogCacheForTests(): void {
  skillsCatalogCache.clear();
  skillsCatalogInflight.clear();
  ensuredZogSkillsDirs.clear();
}

export function zogSkillsDir(zogBaseDir: string): string {
  return nodePath.join(zogBaseDir, "skills");
}

// Creates the portable skills folder on first use so users have a drop-in target.
export async function ensureZogSkillsDir(zogBaseDir: string): Promise<string> {
  const dir = zogSkillsDir(zogBaseDir);
  if (ensuredZogSkillsDirs.has(dir)) {
    return dir;
  }
  try {
    await fs.mkdir(dir, { recursive: true });
    ensuredZogSkillsDirs.add(dir);
  } catch {
...
const SKILL_ORIGIN_ROOTS = {
  zog: {
    homeRoots: (input) => [zogSkillsDir(input.zogBaseDir)],
    projectRootNames: [".zog"],
  },
  codex: {
    // Keep Zog's existing Codex-local root. Official Codex discovery uses
...
// Native copies first so an agent keeps using its own skill, then Zog as the
...
function orderedOriginsForProvider(
  ...
  if (includeZogRoot && !ordered.includes("zog")) {
    ordered.push("zog");
  }
  if (!includeRemainingOrigins) {
    return ordered.filter((origin) => includeZogRoot || origin !== "zog");
  }
  for (const origin of HOME_ORIGIN_ORDER) {
    if (!includeZogRoot && origin === "zog") {
      continue;
    }
...
export function skillsCatalogRoots(input: SkillsCatalogRootInput): SkillRoot[] {
  return rootsForOrderedOrigins(
    input,
    orderedOriginsForProvider(input.provider, input.includeZogRoot !== false),
  );
}
...
export async function discoverSkillsCatalog(
  ...
  const scan = (async () => {
    await ensureZogSkillsDir(input.zogBaseDir);
    const roots = [
