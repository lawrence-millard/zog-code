...
const make = Effect.gen(function* () {
  ...
  const getComposerCapabilities: ProviderDiscoveryServiceShape["getComposerCapabilities"] = (
    ...
    Effect.gen(function* () {
      ...
      // The unified Zog skills catalog backs skill discovery for every
    ...
    Effect.gen(function* () {
      ...
      const nativeResult: ProviderListSkillsResult | null = adapter.listSkills
        ? yield* adapter
            ...
            .pipe(
              Effect.catch((error) =>
                Effect.logWarning(
                  "provider-native skill discovery failed; serving the Zog skills catalog only",
      ...
      const catalogSkills = yield* Effect.tryPromise(() =>
        discoverSkillsCatalog({
          ...
          zogBaseDir: serverConfig.baseDir,
          provider: parsed.provider,
          ...(parsed.forceReload !== undefined ? { forceReload: parsed.forceReload } : {}),
        }),
      ).pipe(
        Effect.catchCause((cause) =>
          Effect.logWarning("zog skills catalog discovery failed", {
            provider: parsed.provider,
            cause,
      ...
      return {
        ...
        source: nativeResult?.source ? `${nativeResult.source}+zog.catalog` : "zog.catalog",
