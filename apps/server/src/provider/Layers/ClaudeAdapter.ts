...
import { buildClaudeSubagentPrompt } from "@zog/shared/agentMentions";
...
import { renderZogHarnessPolicy } from "../../agentGateway/harnessPolicy.ts";
...
// from these hints (see extractSubagentIdentityHints in @zog/shared/subagents).
...
export const buildEmbeddedClaudeSystemPromptAppend = (gatewayControlAvailable: boolean) =>
  [
    "You are running inside Zog, a coding app that embeds the Claude Agent SDK.",
    ...
    renderZogHarnessPolicy({ gatewayControlAvailable }),
  ].join("\n");
        ...
        if (Exit.isFailure(exit)) {
          if (hasPendingUserInterrupt(context) || isClaudeInterruptedCause(exit.cause)) {
            ...
            if (isClaudeMissingResumeConversationCause(exit.cause)) {
              ...
              // starts a fresh Claude session and bootstraps Zog's retained
    ...
    const startSessionUnlocked: ClaudeAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        const settings = {
          ...
          // Zog's safer 200k budget explicit unless the thread opts into 1M.
