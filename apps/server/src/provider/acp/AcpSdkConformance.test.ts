// Verifies the current Zog ACP boundary against an official-SDK subprocess.
...
function createFixtureLog(): string {
  const directory = mkdtempSync(path.join(os.tmpdir(), "zog-acp-conformance-"));
...
function runtimeLayer(logPath: string, env: Record<string, string> = {}) {
  return AcpSessionRuntime.layer({
    spawn: {
      ...
      env: {
        VITEST: "true",
        ZOG_ACP_CONFORMANCE_LOG_PATH: logPath,
        ...env,
      },
    },
    cwd: process.cwd(),
    ...
    clientInfo: { name: "zog-conformance-test", version: "0.0.0" },
    authMethodId: "test",
    authenticateMeta: {
      primitive: 11,
      nested: { source: "zog-auth" },
    },
  });
}
...
describe("official ACP SDK conformance at the current Zog boundary", () => {
    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        runtimeLayer(logPath, {
          ZOG_ACP_CONFORMANCE_MALFORMED_PREFIX: "1",
        }),
      ),
      Effect.scoped,
      Effect.provide(NodeServices.layer),
  ...
  it.effect("cancels an extension request through official-SDK $/cancel_request", () => {
    ...
    return Effect.gen(function* () {
      ...
      const request = yield* runtime
        .request("conformance/wait-for-generic-cancel", { source: "zog" })
        .pipe(Effect.forkChild);
...
describe("official ACP SDK client against the official SDK mock agent", () => {
  test("completes initialize, authentication, session, prompt, updates, stop, and teardown", async () => {
    ...
    const child = spawn(process.execPath, [officialMockFixturePath], {
      ...
      env: {
        ...process.env,
        ZOG_ACP_REQUEST_LOG_PATH: requestLogPath,
        ZOG_ACP_EXIT_LOG_PATH: exitLogPath,
        ZOG_ACP_PROMPT_RESPONSE_TEXT: "mock says héllo 👋",
      },
