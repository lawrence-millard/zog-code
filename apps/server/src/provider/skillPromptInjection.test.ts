...
const zogSkillPath = "/Users/me/.zog/skills/reviewer/SKILL.md";
...
describe("shouldInlineSkillForProvider", () => {
  it("skips codex-native and zog roots for codex but inlines foreign provider roots", () => {
    // Codex loads .codex roots natively and ~/.zog/skills via the extra
    // skill root registered at session start.
    expect(shouldInlineSkillForProvider("codex", zogSkillPath)).toBe(false);
  ...
  it("inlines only Zog-owned paths for cursor", () => {
    expect(shouldInlineSkillForProvider("cursor", zogSkillPath)).toBe(true);
  ...
  it("inlines everything except .claude paths for claudeAgent", () => {
    ...
    expect(shouldInlineSkillForProvider("claudeAgent", zogSkillPath)).toBe(true);
  ...
  it("inlines cross-provider paths for pi but not pi-native skills", () => {
    expect(shouldInlineSkillForProvider("pi", zogSkillPath)).toBe(true);
  ...
  it("always inlines for providers without native skill support", () => {
    for (const provider of ["antigravity", "grok", "kilo", "opencode"] as const) {
      expect(shouldInlineSkillForProvider(provider, zogSkillPath)).toBe(true);
...
describe("buildInlineSkillInstructions", () => {
  it("inlines skill content for non-native providers and skips unreadable paths", async () => {
    ...
    const skillDir = path.join(root, ".zog", "skills", "reviewer");
    try {
      ...
      const text = await buildInlineSkillInstructions({
        ...
        skills: [
          ...
          { name: "missing", path: path.join(root, ".zog", "skills", "missing", "SKILL.md") },
        ],
        maxChars: 10_000,
      });
  ...
  it("does not inline zog-rooted skills for codex (covered by the extra skill root)", async () => {
    const text = await buildInlineSkillInstructions({
      provider: "codex",
      skills: [{ name: "reviewer", path: zogSkillPath }],
      maxChars: 10_000,
    });
    expect(text).toBe("");
  });
