...
import { prewarmChatGptVoiceTranscriptionConnection } from "@zog/shared/chatGptVoiceTranscription";
import { getModelSelectionBooleanOptionValue, normalizeModelSlug } from "@zog/shared/model";
import { decodeSubagentReceiverThreadIds } from "@zog/shared/subagents";
...
import { ZOG_GATEWAY_HARNESS_POLICY } from "./agentGateway/harnessPolicy.ts";
import {
...
Prefer Zog's built-in browser for browser work. It may continue in the background without changing the user's active chat. In code mode, call its MCP methods directly inside \`functions.exec\` with the exact \`tools.mcp__zog__browser_*\` prefix (for example, \`await tools.mcp__zog__browser_open({ url })\` and \`await tools.mcp__zog__browser_snapshot({})\`). The available suffixes are ${BROWSER_TOOL_NAMES.map((name) => `\`${name.slice("browser_".length)}\``).join(", ")}.
...
// Maps Zog's simple runtime toggle to Codex thread-level permission overrides.
...
// Zog re-sends turn-level Codex permission overrides, so keep "always allow"
...
export class CodexAppServerManager extends EventEmitter<CodexAppServerManagerEvents> {
  ...
  private readonly zogSkillsDir: string | undefined;
  ...
  constructor(
    ...
    this.zogSkillsDir = options?.zogSkillsDir;
  ...
  // The Zog MCP server rides on the shared overlay config (no secrets),
  ...
  private async buildSessionProcessEnv(
    ...
    if (gatewayBearerToken) {
      env[ZOG_AGENT_GATEWAY_TOKEN_ENV] = gatewayBearerToken;
    }
    return env;
  }

  // Registers `~/.zog/skills` as a codex skill root so portable skills are
  ...
  private async registerZogSkillsRoot(context: CodexSessionContext): Promise<void> {
    if (!this.zogSkillsDir) {
      return;
    }
    try {
      await this.sendRequest(context, "skills/extraRoots/set", {
        extraRoots: [this.zogSkillsDir],
      });
    } catch (error) {
      // Older codex builds (< extra-roots support) keep working; Zog-only
  ...
  async startSession(input: CodexAppServerStartSessionInput): Promise<ProviderSession> {
    ...
    try {
      ...
      await this.registerZogSkillsRoot(context);
  ...
  private async handleServerRequest(
    ...
    if (isUserInputRequest) {
      ...
      const detail = "Codex asked a question Zog could not render, so it was declined.";
