...
describe("migration backup recovery CLI", () => {
  it("ships the recovery command from the bundled server package", async () => {
    ...
    expect(packageJson.bin?.["zog-restore-migration-backup"]).toBe(
      "dist/restoreMigrationBackup.mjs",
    );
  });

  it("rejects relative database paths and warns operators to stop Zog", async () => {
    ...
    expect(capture.errors.join("\n")).toContain("zog-restore-migration-backup");
    expect(capture.warnings.join("\n")).toContain("Stop every Zog process");
