...
/**
 * A tracker identity that a *released* Zog build wrote for a migration whose
 ...
 * does the migration Zog registers at `historicalId` *today* still need to
 ...
 * once the imported tracker's high-water mark reaches Zog's latest ID,
 * every Zog migration is skipped silently and startup crashes on missing
 ...
 *
 * Instead, compare the recorded (id, name) pairs against Zog's lineage and
 ...
 *
 * Divergences caused by Zog renumbering one of its *own* released
...
export const reconcileMigrationLineage = Effect.gen(function* () {
  ...
  // A released Zog build may have recorded a migration under an ID it no
  ...
  if (aliasRepairs.length > 0) {
    yield* Effect.logInfo(
      "Migration tracker records a renumbered Zog migration; repairing tracker metadata in place",
    ).pipe(
      Effect.annotateLogs({
  ...
  if (diverged === undefined) {
    ...
    // a newer Zog build. Continuing would expose it to stale writable
  ...
  yield* Effect.logWarning(
    "Migration tracker diverges from the Zog lineage (legacy import); re-running migrations from the divergence point",
