...
const TEST_CAPABILITY = "zog-browser-host-client-test-capability-0123456789";

function pipePathForTest(name: string): string {
  return process.platform === "win32"
    ? String.raw`\\.\pipe\zog-${name}-${process.pid}-${crypto.randomUUID()}`
    : path.join("/tmp", `zog-${process.pid}-${crypto.randomUUID().slice(0, 8)}.sock`);
}
...
describe("browser host RPC client", () => {
  ...
  it("prefers the canonical host path and accepts the legacy alias during upgrade", () => {
    expect(
      resolveBrowserHostPipePath({
        ZOG_BROWSER_HOST_PIPE_PATH: "/tmp/canonical.sock",
        ZOG_BROWSER_USE_PIPE_PATH: "/tmp/legacy.sock",
      }),
    ).toBe("/tmp/canonical.sock");
    expect(resolveBrowserHostPipePath({ ZOG_BROWSER_USE_PIPE_PATH: "/tmp/legacy.sock" })).toBe(
      "/tmp/legacy.sock",
    );
  });
  ...
  it("accepts only a bounded private desktop capability from direct test environments", () => {
    expect(
      resolveBrowserHostCapability({
        ZOG_BROWSER_HOST_CAPABILITY: TEST_CAPABILITY,
      }),
    ).toBe(TEST_CAPABILITY);
    expect(
      resolveBrowserHostCapability({
        ZOG_BROWSER_HOST_CAPABILITY: "too-short",
      }),
    ).toBeNull();
  });
});
