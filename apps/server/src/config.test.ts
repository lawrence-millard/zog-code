...
const originalZogStaticDir = process.env.ZOG_STATIC_DIR;

function makeTempDir(prefix = "zog-config-test-"): string {
...
afterEach(() => {
  ...
  tempDirs.clear();
  if (originalZogStaticDir === undefined) {
    delete process.env.ZOG_STATIC_DIR;
  } else {
    process.env.ZOG_STATIC_DIR = originalZogStaticDir;
  }
});

describe("resolveStaticDir", () => {
  it("uses the desktop static snapshot exposed through the Zog environment", async () => {
    const snapshotDir = makeTempDir("zog-static-snapshot-test-");
    fs.writeFileSync(path.join(snapshotDir, "index.html"), "<main>Zog</main>");
    process.env.ZOG_STATIC_DIR = snapshotDir;
...
describe("resolveDefaultChatWorkspaceRoot", () => {
  it("places the managed chat workspace under Documents/Zog on macOS and Linux", () => {
    expect(
      resolveDefaultChatWorkspaceRoot({
        homeDir: "/Users/tester",
        platform: "darwin",
      }),
    ).toBe("/Users/tester/Documents/Zog");
    expect(
      resolveDefaultChatWorkspaceRoot({
        homeDir: "/home/tester",
        platform: "linux",
      }),
    ).toBe("/home/tester/Documents/Zog");
  });
  ...
  it("uses Windows separators when deriving the managed chat workspace on Windows", () => {
    ...
    ).toBe("C:\\Users\\tester\\Documents\\Zog");
  });
...
describe("resolveDefaultStudioWorkspaceRoot", () => {
  it("places the Studio workspace under Documents/Zog/Studio on macOS and Linux", () => {
    expect(
      resolveDefaultStudioWorkspaceRoot({
        homeDir: "/Users/tester",
        platform: "darwin",
      }),
    ).toBe("/Users/tester/Documents/Zog/Studio");
    expect(
      resolveDefaultStudioWorkspaceRoot({
        homeDir: "/home/tester",
        platform: "linux",
      }),
    ).toBe("/home/tester/Documents/Zog/Studio");
  });
  ...
  it("uses Windows separators when deriving the Studio workspace on Windows", () => {
    ...
    ).toBe("C:\\Users\\tester\\Documents\\Zog\\Studio");
  });
});
...
describe("resolveCanonicalWorkspaceRoots", () => {
  it("canonicalizes a symlinked home directory to match project row realpaths", async () => {
    ...
    expect(result.chatWorkspaceRoot).toBe(path.join(expectedHomeDir, "Documents", "Zog"));
    expect(result.studioWorkspaceRoot).toBe(
      path.join(expectedHomeDir, "Documents", "Zog", "Studio"),
    );
  });
  ...
  it("canonicalizes the nearest existing ancestor when the workspace root itself does not exist yet", async () => {
    ...
    // Zog/ nor Zog/Studio exist yet underneath it.
    ...
    expect(result.chatWorkspaceRoot).toBe(path.join(expectedDocuments, "Zog"));
    expect(result.studioWorkspaceRoot).toBe(path.join(expectedDocuments, "Zog", "Studio"));
