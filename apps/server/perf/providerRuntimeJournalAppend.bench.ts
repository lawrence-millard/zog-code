...
const EVENT_COUNT = readPositiveInteger("ZOG_BENCH_EVENTS", 10_000);
const SAMPLE_COUNT = readPositiveInteger("ZOG_BENCH_SAMPLES", 8);
...
function createDatabase(): { readonly database: Database; readonly dispose: () => void } {
  const directory = mkdtempSync(path.join(os.tmpdir(), "zog-runtime-journal-bench-"));
