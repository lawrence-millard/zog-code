// zog-device-helper — the native side of Zog's Device Pane.
//
...
final class HelperSession {
  ...
  private(set) var hid: ZogHIDBridge?
  private(set) var accessibility: ZogAXBridge?
  ...
  func requireHID() throws -> ZogHIDBridge {
  ...
  func requireAccessibility() throws -> ZogAXBridge {
...
///
/// Every injection path in ZogHIDBridge returns silently when it has no
...
private func withHIDDelivery(_ hid: ZogHIDBridge, _ body: () -> Void) throws {
...
func handle(method: String, params: Params, session: HelperSession) throws -> Any {
  ...
  case "button":
    ...
    var button = ZogHardwareButton.home
    guard ZogHardwareButtonFromName(name, &button) else {
      throw RPCError(
        .invalidParams,
...
let requestQueue = DispatchQueue(label: "dev.zog.device-helper.rpc")
