...
id ZogEmptyTranslatorResponse(void);
...
static id ZogMsgSend0(id target, NSString *selectorName) {
...
static NSString *ZogStringAttribute(id element, NSString *selectorName) {
  id value = ZogMsgSend0(element, selectorName);
  return [value isKindOfClass:NSString.class] ? value : nil;
}

static BOOL ZogBoolAttribute(id element, NSString *selectorName) {
  SEL selector = NSSelectorFromString(selectorName);
...
static id ZogJSONValue(id value) {
  if (value == nil) {
    return NSNull.null;
  }
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _callbackQueue = dispatch_queue_create("dev.zog.device-helper.ax", DISPATCH_QUEUE_CONCURRENT);
    _requestTimeout = 5.0;
  }
  return self;
}
...
- (id)accessibilityTranslationDelegateBridgeCallbackWithToken:(NSString *)token {
  ...
  id (^callback)(id) = ^id(id request) {
    if (device == nil) {
      return ZogEmptyTranslatorResponse();
    }
    ...
    return response ?: ZogEmptyTranslatorResponse();
  };
...
- (nullable NSDictionary *)frontmostTreeWithMaxDepth:(NSInteger)maxDepth error:(NSError **)error {
  if (_translator == nil) {
    if (error) {
      *error = [NSError errorWithDomain:@"dev.zog.device-helper.ax"
                                   code:1
...
- (NSDictionary *)serializeElement:(id)element depth:(NSInteger)depth maxDepth:(NSInteger)maxDepth {
  ...
  NSString *role = ZogStringAttribute(element, @"accessibilityRole");
  ...
  NSString *label = ZogStringAttribute(element, @"accessibilityLabel");
  if (label.length > 0) {
    node[@"label"] = label;
  }
  id value = ZogJSONValue(ZogMsgSend0(element, @"accessibilityValue"));
  if (value != NSNull.null) {
    node[@"value"] = value;
  }
  NSString *identifier = ZogStringAttribute(element, @"accessibilityIdentifier");
  if (identifier.length > 0) {
    node[@"identifier"] = identifier;
  }
  NSString *title = ZogStringAttribute(element, @"accessibilityTitle");
  if (title.length > 0) {
    node[@"title"] = title;
  }
  ...
  NSString *subrole = ZogStringAttribute(element, @"accessibilitySubrole");
  ...
  node[@"enabled"] = @(ZogBoolAttribute(element, @"isAccessibilityEnabled"));
  ...
  id rawChildren = ZogMsgSend0(element, @"accessibilityChildren");
