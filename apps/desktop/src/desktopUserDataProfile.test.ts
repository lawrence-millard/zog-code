...
function makeTempDir(): string {
  const directory = FS.mkdtempSync(Path.join(OS.tmpdir(), "zog-profile-test-"));
...
describe("desktopUserDataProfile", () => {
  it("resolves the canonical Zog profile names", () => {
    const appDataBase = "/Users/tester/Library/Application Support";
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "zog-dev" })).toBe(
      "/Users/tester/Library/Application Support/zog-dev",
    );
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "zog" })).toBe(
      "/Users/tester/Library/Application Support/zog",
    );
    expect(
      resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "zog-canary" }),
    ).toBe("/Users/tester/Library/Application Support/zog-canary");
  });
  ...
  it("repairs missing browser data from the profile recorded by the bridge", () => {
    ...
    const targetPath = Path.join(appDataBase, "zog");
    ...
    const targetPartitionPath = Path.join(targetPath, "Partitions", "zog-browser");
    ...
    FS.writeFileSync(
      Path.join(targetPath, "zog-profile-seed.json"),
  ...
  it("rejects bridge manifests that point outside the Zog profile parent", () => {
    const appDataBase = makeTempDir();
  ...
  it("never adds a foreign SQLite sidecar beside an existing Zog database", () => {
    const appDataBase = makeTempDir();
    const targetPath = Path.join(appDataBase, "zog");
    const sourcePath = Path.join(appDataBase, "previous-profile");
    const sourcePartitionPath = Path.join(sourcePath, "Partitions", "previous-browser");
  ...
  it("replaces an orphaned target sidecar with one from the repaired database generation", () => {
    ...
    expect(
      FS.readdirSync(targetPartitionPath).some((entryName) =>
        entryName.startsWith(".zog-bridge-"),
      ),
    ).toBe(false);
  });
  ...
  it("ignores a malformed bridge manifest without attempting a repair", () => {
    ...
    const targetPath = Path.join(appDataBase, "zog");
    FS.mkdirSync(targetPath, { recursive: true });
    FS.writeFileSync(Path.join(targetPath, "zog-profile-seed.json"), "{");
