...
import { PROVIDER_SEND_TURN_MAX_ATTACHMENTS } from "@zog/contracts";
import { ZOG_DEVELOPMENT_BUNDLE_ID } from "@zog/shared/desktopIdentity";
...
describe("desktop AppSnap platform state", () => {
  it("exposes an explicit unsupported state outside macOS", async () => {
    ...
    const manager = new DesktopAppSnapManager({
      platform: "win32",
      helperPath: "C:\\missing\\zog-appsnap-helper.exe",
      captureDirectory: "C:\\tmp\\appsnap",
      excludedBundleId: ZOG_DEVELOPMENT_BUNDLE_ID,
      onState,
      onCaptured: vi.fn(),
      onError: vi.fn(),
    });
  ...
  it("preserves a missing-helper error instead of reporting a permission problem", async () => {
    const manager = new DesktopAppSnapManager({
      platform: "darwin",
      helperPath: "/tmp/zog-appsnap-helper-that-does-not-exist",
      captureDirectory: "/tmp/zog-appsnap-test",
      excludedBundleId: ZOG_DEVELOPMENT_BUNDLE_ID,
      onState: vi.fn(),
      onCaptured: vi.fn(),
      onError: vi.fn(),
...
describe("AppSnap helper protocol", () => {
  ...
  it("coalesces concurrent listener reconciliation while the capture directory is prepared", async () => {
    const captureDirectory = mkdtempSync(join(tmpdir(), "zog-appsnap-reconcile-"));
  ...
  it("retains a full composer batch and reports any overflow", async () => {
    const captureDirectory = mkdtempSync(join(tmpdir(), "zog-appsnap-test-"));
    const checkChild = createFakeChildProcess();
    const watchChild = createFakeChildProcess();
    const spawn = vi
      .fn()
  ...
  it("keeps the helper capture file when persisting the pending copy fails", async () => {
    const captureDirectory = mkdtempSync(join(tmpdir(), "zog-appsnap-persist-fail-"));
    const checkChild = createFakeChildProcess();
    const watchChild = createFakeChildProcess();
    const spawn = vi
      .fn()
  ...
  it("restores pending captures after a manager restart and removes them only after ack", async () => {
    const captureDirectory = mkdtempSync(join(tmpdir(), "zog-appsnap-pending-"));
    const checkChild = createFakeChildProcess();
    const watchChild = createFakeChildProcess();
    const spawn = vi
      .fn()
  ...
  it("recovers a helper PNG left behind before pending metadata was persisted", async () => {
    const captureDirectory = mkdtempSync(join(tmpdir(), "zog-appsnap-helper-recovery-"));
