...
import type { BrowserToolName, ThreadId } from "@zog/contracts";
...
const PIPE_DIR = "zog-browser-host";
const PIPE_NAME_PREFIX = "zog-browser-host";

export const ZOG_BROWSER_HOST_PIPE_ENV = "ZOG_BROWSER_HOST_PIPE_PATH";
export const ZOG_BROWSER_HOST_CAPABILITY_ENV = "ZOG_BROWSER_HOST_CAPABILITY";
export const ZOG_BROWSER_HOST_CAPABILITY_FD_ENV = "ZOG_BROWSER_HOST_CAPABILITY_FD";
/** @deprecated Read/written only while old backend builds are still supported. */
export const ZOG_BROWSER_USE_PIPE_ENV = "ZOG_BROWSER_USE_PIPE_PATH";
...
export function resolveConfiguredBrowserHostPipePath(
  ...
  const configured =
    env[ZOG_BROWSER_HOST_PIPE_ENV]?.trim() || env[ZOG_BROWSER_USE_PIPE_ENV]?.trim();
...
export const ZOG_BROWSER_HOST_PIPE_PATH = resolveConfiguredBrowserHostPipePath();
/** @deprecated Compatibility alias for old packaged backend builds. */
export const ZOG_BROWSER_USE_PIPE_PATH = ZOG_BROWSER_HOST_PIPE_PATH;
...
export function resolveBrowserHostPipeBackendEnv(
  ...
  delete backendEnv[ZOG_BROWSER_HOST_PIPE_ENV];
  delete backendEnv[ZOG_BROWSER_USE_PIPE_ENV];
  delete backendEnv[ZOG_BROWSER_HOST_CAPABILITY_ENV];
  delete backendEnv[ZOG_BROWSER_HOST_CAPABILITY_FD_ENV];
  const pipePath = activePipePath?.trim();
  if (pipePath && Number.isInteger(capabilityFd) && (capabilityFd ?? 0) >= 3) {
    backendEnv[ZOG_BROWSER_HOST_PIPE_ENV] = pipePath;
    backendEnv[ZOG_BROWSER_USE_PIPE_ENV] = pipePath;
    backendEnv[ZOG_BROWSER_HOST_CAPABILITY_FD_ENV] = String(capabilityFd);
  }
  return backendEnv;
}
...
export class BrowserHostPipeServer {
  ...
  constructor(
    ...
    options: BrowserHostPipeServerOptions | string = ZOG_BROWSER_HOST_PIPE_PATH,
  ) {
    const normalized = typeof options === "string" ? { pipePath: options } : options;
    this.platform = normalized.platform ?? process.platform;
    this.pipePath = normalized.pipePath ?? ZOG_BROWSER_HOST_PIPE_PATH;
  ...
  private getInfo(client: PipeClient, params: unknown): unknown {
    ...
    return {
      name: "Zog Browser Host",
      version: "1.0.0",
      type: "zog-browser-host",
      metadata: {
        sessionId,
        protocolVersion: 1,
