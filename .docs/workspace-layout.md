...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `zog` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@zog/shared/git`, `@zog/shared/DrainableWorker`) — no barrel index.
