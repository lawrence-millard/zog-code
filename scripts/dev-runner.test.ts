...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit ZOG_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "ZOG_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid ZOG_PORT_OFFSET"));
      }),
    );
  });
  ...
  describe("boolean precedence", () => {
    ...
    it.effect("rejects invalid boolean environment values", () =>
      Effect.gen(function* () {
        const error = yield* Effect.flip(
          readDevRunnerBooleanEnvironment({ ZOG_LOG_WS_EVENTS: "sometimes" }),
        );
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("marks an inherited terminal PATH as already hydrated", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.ZOG_PATH_HYDRATED, "1");
        assert.match(env.PATH ?? "", /\/opt\/homebrew\/bin/);
      }),
    );

    it.effect("defaults ZOG_HOME to ~/.zog when not provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.ZOG_HOME, resolve(homedir(), ".zog"));
        assert.equal(env.ZOG_HOST, "127.0.0.1");
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.ZOG_HOME, resolve("/tmp/custom-zog"));
        assert.equal(env.ZOG_PORT, "4222");
        assert.equal(env.ZOG_NO_BROWSER, "1");
        assert.equal(env.ZOG_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.ZOG_LOG_WS_EVENTS, "1");
        assert.equal(env.ZOG_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.ZOG_MODE, "web");
        assert.equal(env.ZOG_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.ZOG_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom zogHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.ZOG_HOME, resolve("/tmp/my-zog"));
        assert.equal(env.ZOG_HOME, resolve("/tmp/my-zog"));
        assert.equal(env.ZOG_HOME, resolve("/tmp/my-zog"));
      }),
    );
