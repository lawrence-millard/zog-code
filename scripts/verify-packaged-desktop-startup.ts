...
function prepareWindowsLaunch(assetsDirectory: string, extractionRoot: string): LaunchCommand {
  ...
  const executables = findFiles(applicationRoot, (candidate) =>
    /[/\\]Zog\.exe$/i.test(candidate),
  );
  if (executables.length !== 1) {
    throw new Error(`Expected one extracted Zog.exe, found ${executables.length}.`);
  }
...
export function createPackagedDesktopSmokeEnvironment(
  ...
  if (options.platform === "mac") {
    const userDataPath = join(env.HOME!, "Library", "Application Support", "zog");
...
export async function verifyPackagedDesktopStartup(
  ...
  const temporaryRoot = mkdtempSync(join(tmpdir(), `zog-packaged-smoke-${options.platform}-`));
  ...
  try {
    ...
    const logPath = join(env.ZOG_HOME!, "userdata", "logs", "desktop-main.log");
