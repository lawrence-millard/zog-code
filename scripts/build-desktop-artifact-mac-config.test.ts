...
describe("createDesktopPlatformBuildConfig", () => {
  it("adds explicit microphone entitlements to macOS builds", () => {
    ...
    assert.equal(MAC_APPSNAP_HELPER_BUNDLE_PATH, "Contents/Helpers/zog-appsnap-helper");
    assert.deepStrictEqual(mac.binaries, ["Contents/Helpers/zog-appsnap-helper"]);
    assert.equal(mac.x64ArchFiles, "Contents/Helpers/zog-appsnap-helper");
    assert.equal(
      MAC_APPSNAP_HELPER_STAGE_PATH,
      "apps/desktop/native/appsnap/build/zog-appsnap-helper",
    );
    ...
    assert.deepStrictEqual(config.extraFiles, [
      {
        from: "apps/desktop/native/appsnap/build/zog-appsnap-helper",
        to: "Helpers/zog-appsnap-helper",
      },
      {
  ...
  it("leaves non-macOS platform configs unchanged", () => {
    ...
    const win = createDesktopPlatformBuildConfig({
      platform: "win",
      target: "nsis",
      windowsAzureSignOptions: { publisherName: "Zog" },
    });
    ...
    assert.deepStrictEqual(win.win, {
      ...
      icon: "icon.ico",
      publisherName: "Zog",
      azureSignOptions: { publisherName: "Zog" },
    });
  });
