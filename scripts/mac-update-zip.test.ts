...
`,
  ...
  it("builds Electron framework symlink paths for the top-level app bundle", () => {
    assert.deepStrictEqual(buildMacUpdateZipSymlinkEntries("Zog.app"), [
      "Zog.app/Contents/Frameworks/Electron Framework.framework/Electron Framework",
      "Zog.app/Contents/Frameworks/Electron Framework.framework/Helpers",
      "Zog.app/Contents/Frameworks/Electron Framework.framework/Libraries",
      "Zog.app/Contents/Frameworks/Electron Framework.framework/Resources",
      "Zog.app/Contents/Frameworks/Electron Framework.framework/Versions/Current",
    ]);
  });
  ...
  it("resolves exactly one top-level .app from update zip entries", () => {
    assert.equal(
      resolveSingleTopLevelMacAppBundle([
        "__MACOSX/Zog.app/Contents/Info.plist",
        "Zog.app/Contents/Info.plist",
        "Zog.app/Contents/MacOS/Zog",
      ]),
      "Zog.app",
    );
  ...
  it("resolves exactly one macOS update zip artifact", () => {
    ...
    assert.throws(
      () => resolveSingleMacUpdateZipFileName(["Zog-0.1.5-arm64.zip", "Zog-0.1.5-x64.zip"]),
  ...
  it("requires at least one macOS update manifest", () => {
    ...
    assert.throws(
      () => resolveMacUpdateManifestFileNames(["Zog-0.1.5-arm64.dmg"]),
...
files:
  - url: Zog-0.1.4-arm64.zip
    sha512: oldzip
    size: 100
  - url: Zog-0.1.4-arm64.dmg
    sha512: olddmg
    size: 200
path: 'Zog-0.1.4-arm64.zip'
sha512: oldzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;

    const updated = updateMacUpdateManifestZipEntry(manifest, "Zog-0.1.4-arm64.zip", {
      sha512: "newzip",
      size: 12345,
    });
...
`,
      ...
      /Could not update Zog-0.1.4-arm64.zip entry/,
    );
  });
...
files:
  - url: Zog-0.1.5-arm64.zip
    sha512: newzip
    size: 12345
path: Zog-0.1.5-arm64.zip
sha512: newzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;
    ...
    assert.deepStrictEqual(
      validateMacUpdateManifestZipMetadata(manifest, "Zog-0.1.5-arm64.zip", metadata),
      {
    ...
    assert.deepStrictEqual(
      assertMacUpdateManifestZipMetadata(manifest, "Zog-0.1.5-arm64.zip", metadata),
      {
        manifestHasZipPath: true,
        manifestHasZipSha: true,
        manifestHasZipSize: true,
