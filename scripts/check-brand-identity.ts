...
const approvedAttributions: readonly ApprovedAttribution[] = [
  ...
  },
  {
    ...
    line: `Zog began as a clone of [${retiredFirstDisplayName}](https://github.com/pingdotgg/${retiredFirstName}), but it has since become a substantially different product with its own branding, packaging, release system, provider orchestration, desktop app behavior, and product direction.`,
  },
  {
    path: "CHANGELOG.md",
    markdownSection: "## 0.7.0 - 2026-08-05",
    line: `**A review of the Zog codebase found an analytics configuration that came from the original ${retiredFirstSpacedDisplayName} codebase when Zog was created as a clone in March. We did not add it, and we have no access to the PostHog project receiving the events.**`,
  },
  {
    path: "apps/web/src/whatsNew/entries.ts",
    line: `"A review of the Zog codebase found an analytics configuration that came from the original ${retiredFirstSpacedDisplayName} codebase when Zog was created as a clone in March.",`,
  },
];
...
function main(): void {
  ...
  if (violations.length === 0) {
    console.log("Zog identity check passed.");
    return;
  }
