...
You are GPT-5.5 (xhigh) in the Zog monorepo at `/Users/emanueledipietro/Developer/zog`.
...
- `config.homeDir` = `OS.homedir()` ALWAYS (`apps/server/src/main.ts:172,217`) — it is NOT affected by
  `--home-dir`.
- `config.baseDir` = `--home-dir` / `ZOG_HOME` (dev: `./.zog/electron-dev`; prod: `~/.zog` or
  `~/Documents/Zog`). `config.dbPath` = `baseDir/{dev|userdata}/state.sqlite`.
...
- The dev runner sets `ZOG_HOME/ZOG_HOME/ZOG_HOME=baseDir` but does NOT set `CODEX_HOME` or
...
5. **Run the checks** to back every claim:
   - `sqlite3` on `./.zog/electron-dev/dev/state.sqlite` (dev) and `~/.zog/userdata/state.sqlite`
     (prod) where relevant.
