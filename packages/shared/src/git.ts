 ...
 */
export const WORKTREE_BRANCH_PREFIX = "zog";
...
const ZOG_BRANCH_FALLBACK = "update";
...
export function buildZogBranchName(preferredBranch?: string | null): string {
  const preferred = preferredBranch?.trim() ?? "";
  ...
  return `${WORKTREE_BRANCH_PREFIX}/${sanitizeBranchFragment(
    normalizedExisting || ZOG_BRANCH_FALLBACK,
  )}`;
}

export function resolveUniqueZogBranchName(
  existingBranchNames: readonly string[],
  preferredBranch?: string | null,
): string {
  const resolvedBase = buildZogBranchName(preferredBranch);
  return resolveUniqueBranchName(existingBranchNames, resolvedBase);
}
